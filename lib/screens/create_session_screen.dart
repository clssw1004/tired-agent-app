...
String _generateDefaultLabel() {
  final rnd = List.generate(8, (_) => _labelChars[Random().nextInt(_labelChars.length)]).join();
...
class _Preset {
  ...
  const _Preset({required this.id, required this.label, required this.cmd, this.args = const [], required this.hint, required this.emoji, this.options = const []});
}
...
class _Option {
  ...
  final List<String> args;
  final String hint;
  const _Option({required this.id, required this.label, required this.args, required this.hint});
}

const _presets = [
  _Preset(id: 'claude', label: 'Claude', cmd: 'claude', hint: 'Anthropic Claude Code CLI', emoji: '✦'),
  _Preset(id: 'bash', label: 'Bash', cmd: 'bash', hint: 'POSIX shell', emoji: r'$', options: [
    _Option(id: 'interactive', label: 'Interactive', args: ['-i'], hint: 'Force interactive mode'),
    _Option(id: 'login', label: 'Login', args: ['-l'], hint: 'Start as a login shell'),
  ]),
  _Preset(id: 'zsh', label: 'Zsh', cmd: 'zsh', hint: 'Z shell', emoji: r'$', options: [
    _Option(id: 'interactive', label: 'Interactive', args: ['-i'], hint: 'Force interactive mode'),
    _Option(id: 'login', label: 'Login', args: ['-l'], hint: 'Start as a login shell'),
  ]),
  _Preset(id: 'cmd', label: 'cmd.exe', cmd: 'cmd.exe', hint: 'Windows command prompt', emoji: '>', options: [
    _Option(id: 'no-auto-run', label: 'No AutoRun', args: ['/d'], hint: 'Disable AutoRun commands'),
  ]),
  _Preset(id: 'powershell', label: 'PowerShell', cmd: 'powershell.exe', hint: 'Windows PowerShell', emoji: '>', options: [
    _Option(id: 'no-logo', label: 'No logo', args: ['-NoLogo'], hint: 'Hide startup logo'),
    _Option(id: 'no-profile', label: 'No profile', args: ['-NoProfile'], hint: 'Skip profile scripts'),
  ]),
  _Preset(id: 'python', label: 'Python', cmd: 'python3', args: ['-i'], hint: 'Interactive Python REPL', emoji: '🐍', options: [
    _Option(id: 'interactive', label: 'Interactive', args: ['-i'], hint: 'Force interactive mode'),
  ]),
  _Preset(id: 'node', label: 'Node', cmd: 'node', hint: 'Node.js REPL', emoji: '⬢', options: [
    _Option(id: 'interactive', label: 'Interactive', args: ['-i'], hint: 'Force interactive mode'),
  ]),
];
...
class _CreateSessionScreenState extends State<CreateSessionScreen> {
  ...
  _Preset? get _selectedPreset => _presets.where((p) => p.cmd == _cmd).firstOrNull;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (ref == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Not authenticated'), backgroundColor: AppColors.danger),
          );
        }
        return;
      }

      final manualArgs = _argsController.text.trim().split(RegExp(r'\s+')).where((s) => s.isNotEmpty).toList();
      ...
      final session = await auth.authService.transport.createSession(ref, spec, agentId: widget.serverId);
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                separatorBuilder: (_, __) => const SizedBox(width: AppSpacing.two),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ThemedText.small(p.label, color: active ? AppColors.text : AppColors.textSecondary),
                        ],
                      ),
                    ),
                  );
            ...
            Row(
              children: [
                ...
                _modeChip(SessionMode.persistent, 'Persistent', 'Manual kill only', available: _cmd == 'claude'),
              ],
            ),
            if (_cmd != 'claude')
              Padding(
                padding: const EdgeInsets.only(top: AppSpacing.one),
                child: ThemedText.small('Persistent mode only available for Claude', color: AppColors.textSecondary),
              ),
            const SizedBox(height: AppSpacing.four),
            ...
            TextField(
              ...
              style: const TextStyle(fontFamily: 'monospace', color: AppColors.text),
              decoration: const InputDecoration(isDense: true, contentPadding: EdgeInsets.symmetric(horizontal: AppSpacing.three, vertical: AppSpacing.two)),
            ),
            ...
            TextField(
              ...
              style: const TextStyle(fontFamily: 'monospace', color: AppColors.text),
              decoration: const InputDecoration(hintText: '--no-input', isDense: true, contentPadding: EdgeInsets.symmetric(horizontal: AppSpacing.three, vertical: AppSpacing.two)),
              enabled: !_busy,
            ),

            // ── Option chips ─────────────────────────────────────────
            if (_selectedPreset?.options != null && _selectedPreset!.options.isNotEmpty) ...[
              const SizedBox(height: AppSpacing.two),
              Wrap(
                ...
                children: _selectedPreset!.options.map((opt) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: ThemedText.small(opt.label, color: active ? AppColors.text : AppColors.textSecondary),
                    ),
                  );
                }).toList(),
              ),
            ...
            // ── Command preview ──────────────────────────────────────
            if (_effectiveArgs.isNotEmpty || _argsController.text.trim().isNotEmpty) ...[
              const SizedBox(height: AppSpacing.two),
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: AppSpacing.two),
                    Expanded(
                      child: ThemedText.code([_cmd, ..._effectiveArgs, ...(_argsController.text.trim().split(RegExp(r'\s+')).where((s) => s.isNotEmpty))].join(' ')),
                    ),
                  ],
                ),
              ),
            ...
            TextField(
              ...
              decoration: const InputDecoration(hintText: 'Leave empty to auto-generate', isDense: true, contentPadding: EdgeInsets.symmetric(horizontal: AppSpacing.three, vertical: AppSpacing.two)),
              enabled: !_busy,
            ),
            const SizedBox(height: AppSpacing.three),
            ...
            const SizedBox(height: AppSpacing.two),
            ThemedText.small('Terminal size auto-matches browser window after session starts', color: AppColors.textSecondary),
  ...
  Widget _modeChip(SessionMode m, String label, String desc, {bool available = true}) {
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.all(AppSpacing.three),
          decoration: BoxDecoration(
            color: active ? AppColors.accent.withAlpha(30) : AppColors.backgroundElement,
            borderRadius: BorderRadius.circular(AppSpacing.two),
            border: Border.all(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ThemedText.body(label, color: active ? AppColors.accent : (available ? AppColors.text : AppColors.textSecondary)),
              const SizedBox(height: AppSpacing.one),
              ThemedText.small(desc, color: available ? AppColors.textSecondary : AppColors.textSecondary.withAlpha(100)),
            ],
          ),
        ),
      ),
